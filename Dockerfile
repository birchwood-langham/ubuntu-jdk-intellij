...
RUN git clone https://github.com/powerline/fonts.git && \
    ...
    mkdir idea && \
    wget https://download-cf.jetbrains.com/idea/ideaIU-2018.1-no-jdk.tar.gz && \
    tar -C idea -zxf ideaIU-2018.1-no-jdk.tar.gz --strip-components=1 && \
    rm ideaIU-2018.1-no-jdk.tar.gz
